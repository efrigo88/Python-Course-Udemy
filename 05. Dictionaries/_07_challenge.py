...
while True:
    ...
    direction = input('Available exits are: {}. Select one: '
                      .format(availableExits)).upper()
    print()

    # Parse the user input, using our vocabulary dictionary if necessary
    if len(direction) > 1:  # check if user entered more than one character
        for word in vocabulary:     # we check this in vocabulary
            if word in direction:
                # if we got a match, we assign the value
